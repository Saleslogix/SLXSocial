...
public partial class SmartParts_Activity_ActivityDetails : EntityBoundSmartPartInfoProvider
{
    ...
    private bool IsInsertMode
    {
        get
        {
            IPersistentEntity persistentEntity = (IPersistentEntity)BindingSource.Current;
            _insertMode = false;
            if (persistentEntity != null)
            {
                _insertMode = ((persistentEntity.PersistentState & PersistentState.New) > 0);
            }
            return _insertMode;
        }
    }
    ...
    protected void TicketId_LookupResultValueChanged(object sender, EventArgs e)
    {
        ...
        if (ticket != null)
        {
            if (Activity.AccountId != ticket.Account.Id.ToString())
            {
    ...
    protected void OpportunityId_LookupResultValueChanged(object sender, EventArgs e)
    {
        ...
        if (opportunity != null)
        {
            if (Activity.AccountId != opportunity.Account.Id.ToString())
            {
    ...
    private void SetFollowupCarryovers()
    {
        CreateCarryOverAttachments();
        SetCarryOverNotes();
        SetCarryOverDescription();
    }

    private void SetCarryOverDescription()
    {
        if (GetParam("description") != null && IsInsertMode)
        {
    ...
    {
        string notes = GetParam("notes");
        if (!string.IsNullOrEmpty(notes))
        {
            Activity.LongNotes = notes;
        }
        string twitterid = GetParam("twitterid");
        if (!string.IsNullOrEmpty(twitterid))
        {
            IList<IContact> contacts = EntityFactory.GetRepository<IContact>().FindByProperty("TwitterId", twitterid);
            if (contacts.Count > 0)
            {
                Activity.ContactId = contacts[0].Id.ToString();
                Activity.AccountId = contacts[0].Account.Id.ToString();
            }
        }
    }
    #endregion
