...
public partial class SmartParts_History_InsertNote : EntityBoundSmartPart
{
    ...
    {
        IHistory hist = BindingSource.Current as IHistory;
        if (hist != null)
        {
            string notes = GetParam("notes");
            if (!string.IsNullOrEmpty(notes))
            {
                hist.LongNotes = notes;
            }
            ...
            if (!string.IsNullOrEmpty(twitterid))
            {
                ...
                if (contacts.Count > 0)
                {
                    hist.ContactId = contacts[0].Id.ToString();
                    hist.AccountId = contacts[0].Account.Id.ToString();
                }
            }
        }
    }
    ...
    private bool IsInsert
    {
        get
        {
            History hist = BindingSource.Current as History;
            return hist != null && (hist.PersistentState & PersistentState.New) > 0;
        }
    }
