...
function (declare, lang, _Widget, TextBox, ComboBox, _TemplatedMixin, _WidgetsInTemplateMixin, Resources) {
    ...
    return declare([_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {
        templateString: "<div class='s-filter'>" +
            "<div data-dojo-type='dijit.form.TextBox' data-dojo-attach-point='txtFilter' style='min-width: 250px'></div>" +
            "<div data-dojo-type='dijit.form.Button' data-dojo-attach-point='btnSearch'>" + Resources.searchText + "</div>" +
            "</div>",
        // base properties for the filter object
        baseFilter: null,
        ...
        setCurrentFilter: function (filter) {
            // used to display effective filter values
            this.txtFilter.set('value', filter.query);
            this.baseFilter = filter;
        },

        _applyFilter: function () {
            var filter = { query: this.txtFilter.get('value') };
            if (this.baseFilter)
                filter = lang.mixin(lang.clone(this.baseFilter), filter);
